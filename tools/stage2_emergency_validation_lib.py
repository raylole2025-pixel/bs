...
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "results" / "stage2"
...
class LoadedStage1Artifacts:
    ...
    def resolve_selected_plan(self) -> tuple[list[ScheduledWindow], list[dict[str, Any]], dict[str, Any] | None]:
        if not self.selected_plan:
            raise ValueError(f"No selected_plan found in {self.stage1_result_path}")
        solution = dict(self.selected_solution) if self.selected_solution is not None else None
        return list(self.selected_plan), list(self.selected_plan_rows), solution
...
def load_stage1_artifacts(stage1_result_path: str | Path) -> LoadedStage1Artifacts:
    ...
    if not selected_plan_rows:
        raise ValueError(f"No selected_plan found in {path}")
    ...
    return LoadedStage1Artifacts(
        ...
        selected_solution=(dict(selected_solution) if isinstance(selected_solution, dict) else None),
...
def _normalize_emergency_source_tasks(raw_tasks: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    rows = [dict(item) for item in raw_tasks]
    if not rows:
        return []
    typed_rows = [normalize_task_payload(row, force_task_type=None) for row in rows]
    explicit_emergencies = [row for row in typed_rows if str(row.get("type", "")).strip().lower() == "emg"]
    if explicit_emergencies:
        return explicit_emergencies
    return [normalize_task_payload(row) for row in rows]
...
def load_emergency_tasks_from_json(path: str | Path) -> list[dict[str, Any]]:
    ...
    return _normalize_emergency_source_tasks(raw_tasks)
...
def load_emergency_tasks_from_csv(path: str | Path) -> list[dict[str, Any]]:
    ...
    return _normalize_emergency_source_tasks(rows)
...
def load_emergency_tasks_from_workbook(path: str | Path, sheet: str) -> list[dict[str, Any]]:
    ...
    return _normalize_emergency_source_tasks(workbook_task_to_payload(row) for row in task_sets[sheet])


def build_stage2_effective_payload(
    *,
...
def _selected_solution_summary(
    *,
    solution_info: dict[str, Any] | None,
    ...
    if solution_info:
        for key in (
            ...
            if key in solution_info:
                summary[key] = solution_info[key]
    return summary
...
def _build_effective_scenario_payload(
    *,
    ...
    payload = build_stage2_effective_payload(base_payload=base_payload, emergency_tasks=emergency_tasks)
...
def build_summary_markdown(summary: dict[str, Any]) -> str:
    ...
    for case in summary["cases"]:
        lines.append(
            f"| {case['case_id']} | {case['case_type']} | {case['rho_label']} | {case['emergency_task_set']['summary']['count']} | {case['stage2']['cr_emg']:.4f} | {case['baseline_impact']['cr_reg_before']:.4f} | {case['baseline_impact']['cr_reg_after']:.4f} | {case['stage2']['n_preemptions']} | {len(case['baseline_impact']['regular_task_degradation'])} |"
        )
    return "\n".join(lines) + "\n"
...
def run_stage2_emergency_validation(
    *,
    ...
    plan, plan_rows, selected_solution_info = stage1_artifacts.resolve_selected_plan()
    ...
    for case_spec in resolved_cases:
        ...
        for rho_variant in resolved_rho_values:
            ...
            case_id = "__".join((_sanitize_slug(case_name), rho_label))
            ...
            use_stage1_trace = (
                stage1_artifacts.baseline_trace is not None
            ...
            selected_solution = _selected_solution_summary(
                solution_info=selected_solution_info,
                plan=plan,
