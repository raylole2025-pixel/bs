...
更新时间：2026-04-22（对应当前阶段一 + 阶段二整理后）。
...
### 1.3 `completion_tolerance` 说明

- `stage1.completion_tolerance` 用于阶段一判断常态任务是否达到“视为完成”的数值容差。
- `stage2.completion_tolerance` 用于阶段二判断回放/抢占后的任务完成容差。
- 若场景未显式提供 `stage2.completion_tolerance`，加载器会默认复用 `stage1.completion_tolerance`。
...
### 3.3 `apps/run_stage1_json_taskset.py`

- `--base-scenario=inputs/templates/stage1_scenario_template.json`
- `--output-root=results/stage1/json_taskset_runs`
- `--seed=7`
- 其余 Stage1、GA 和距离补全默认值与 `apps/run_stage1_workbook_batch.py` 保持一致
...
- 阶段二当前只处理临机任务插入，不再包含 full MILP 或 rolling MILP 主链。
- 阶段二以阶段一正式产物 `selected_plan + selected_solution + baseline_trace` 为交接面。
...
- Stage1 对外结果契约使用 `selected_plan`、`selected_solution`、`baseline_trace`，不再对阶段二暴露候选解回放接口。
