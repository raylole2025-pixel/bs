...
This document summarizes the parameters used by the current codebase after the Stage1/Stage2 cleanup on 2026-04-22.
...
Stored under `stage1` in the scenario JSON and parsed by `bs3/scenario.py`.
...
| `completion_tolerance` | `1e-6` | Completion tolerance ratio for judging whether a regular task is effectively complete. |
...
These are the formal Stage1 result metrics written into retained Stage1 outputs.
...
These come from `apps/run_stage1_workbook_batch.py` and are not part of the core scenario model unless written back into the generated scenario/result files.
...
| --- | --- | --- |
| `elite_count = ceil(0.1P)` | `bs3/models.py` | Number of elites copied each generation. |
| `immigrant_count = ceil(0.1P)` | `bs3/models.py` | Number of immigrants injected each generation. |
| `TOURNAMENT_SIZE = 3` | `bs3/stage1.py` | Tournament selection sample size. |
| `PRUNE_EXACT_LIMIT = 8` | `bs3/stage1.py` | Max exact pruning validations attempted per pruning round. |
