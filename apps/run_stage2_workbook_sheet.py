...
from bs3.stage1_taskset_runner import task_stats
from bs3.stage2 import run_stage2
from apps.run_stage1_workbook_batch import read_task_sets, workbook_task_to_payload
from tools.stage2_emergency_validation_lib import build_stage2_effective_payload, ensure_unique_task_ids, load_emergency_tasks_from_workbook

EPS = 1e-9
PROJECT_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "results" / "stage2" / "workbook_runs"
...
def load_stage1_artifacts(stage1_result_path: Path) -> tuple[list[ScheduledWindow], dict[str, Any] | None, Stage1BaselineTrace | None]:
    ...
    return plan, (dict(selected_solution) if isinstance(selected_solution, dict) else None), baseline_trace
...
def build_scenario_payload(
    ...
    payload = build_stage2_effective_payload(
        ...
        metadata_updates={
            ...
            "effective_emergency_task_count": len(emergency_tasks),
        },
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run stage2 emergency insertion using stage1 selected_plan + an emergency task sheet.")
    ...
    parser.add_argument(
        "--stage1-result",
        required=True,
        help="Stage1 result JSON containing the final selected_plan and baseline_trace",
    )
    ...
    source_tasks = [workbook_task_to_payload(row) for row in workbook_rows]
    emergency_tasks = load_emergency_tasks_from_workbook(workbook_path, args.sheet)
    ...
    stats = task_stats(emergency_tasks)
    plan, selected_solution, baseline_trace = load_stage1_artifacts(stage1_result_path)

    scenario_payload = build_scenario_payload(base_payload, workbook_path, args.sheet, emergency_tasks, args)
    scenario_path = set_dir / f"{args.sheet}_scenario_stage2_input.json"
    write_json(scenario_path, scenario_payload)
    write_json(
        set_dir / f"{args.sheet}_emergency_tasks.json",
        {
            "source_sheet_task_count": len(source_tasks),
            "extracted_emergency_task_count": len(emergency_tasks),
