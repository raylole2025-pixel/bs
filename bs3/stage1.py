...
class RegularEvaluator:
    def __init__(self, scenario: Scenario) -> None:
        ...
        self._summary_cache: dict[tuple[tuple[tuple[str, float, float], ...], float], SimulationSummary] = {}
    ...
    def _completion_tolerance(self, task: Task) -> float:
        return max(float(self.scenario.stage1.completion_tolerance) * float(task.data), EPS)
    ...
    def _simulate(self, plan: list[ScheduledWindow], rho: float, capture_details: bool = False) -> SimulationSummary | SimulationTrace:
        empty_metrics = EvaluationMetrics(mean_completion_ratio=1.0, fr=1.0, eta_cap=0.0, eta_0=0.0)
        if not self.regular_tasks:
            if not capture_details:
                return SimulationSummary(metrics=empty_metrics, window_flow={})
        ...
        segment_rows: list[dict] | None = [] if capture_details else None
        task_state_rows: list[dict] | None = [] if capture_details else None
        allocation_rows: list[Allocation] | None = [] if capture_details else None
        remaining_before_by_segment: dict[str, dict[int, float]] | None = (
            {task.task_id: {} for task in self.baseline_regular_tasks}
            if capture_details
            else None
        )
        remaining_after_by_segment: dict[str, dict[int, float]] | None = (
            {task.task_id: {} for task in self.baseline_regular_tasks}
            if capture_details
            else None
        )
        cross_window_usage_by_segment: dict[int, dict[str, float]] | None = {} if capture_details else None
        available_cross_capacity_by_segment: dict[int, dict[str, float]] | None = {} if capture_details else None
        occupied_cross_windows_by_segment: dict[int, list[str]] | None = {} if capture_details else None
        active_cross_windows_by_segment: dict[int, list[str]] | None = {} if capture_details else None
        active_intra_edges_by_segment: dict[int, dict[str, list[str]]] | None = {} if capture_details else None
        available_intra_capacity_by_segment: dict[int, dict[str, float]] | None = {} if capture_details else None
        ...
        for segment in segments:
            ...
            if capture_details:
                assert remaining_before_by_segment is not None
                for task in self.baseline_regular_tasks:
            ...
            for task in active_tasks:
                ...
                if capture_details:
                    assert allocation_rows is not None
            ...
            if capture_details:
                assert remaining_before_by_segment is not None
                assert remaining_after_by_segment is not None
                assert task_state_rows is not None
                for task in self.baseline_regular_tasks:
                    task_id = task.task_id
                ...
                assert active_cross_windows_by_segment is not None
                assert cross_window_usage_by_segment is not None
                assert available_cross_capacity_by_segment is not None
                assert occupied_cross_windows_by_segment is not None
                assert active_intra_edges_by_segment is not None
                assert available_intra_capacity_by_segment is not None
                assert segment_rows is not None
        ...
        task_rows: list[dict] | None = [] if capture_details else None
        for task in self.baseline_regular_tasks:
        ...
        if not capture_details:
            return SimulationSummary(metrics=metrics, window_flow=dict(window_flow))
    ...
    def _cached_summary(self, plan: list[ScheduledWindow], rho: float | None = None) -> SimulationSummary:
        ...
        if cache_key not in self._summary_cache:
            summary = self._simulate(plan, rho=rho, capture_details=False)
    ...
    def _baseline_trace_from_trace(self, trace: SimulationTrace, rho: float) -> Stage1BaselineTrace:
    ...
    def baseline_trace(self, plan: list[ScheduledWindow], rho: float | None = None) -> Stage1BaselineTrace:
        rho = self.scenario.stage1.rho if rho is None else rho
        trace = self._simulate(plan, rho=rho, capture_details=True)
        assert isinstance(trace, SimulationTrace)
        return self._baseline_trace_from_trace(trace, rho=rho)
    ...
    def trace(self, plan: list[ScheduledWindow], rho: float | None = None, k_paths: int | None = None) -> dict:
        ...
        trace = self._simulate(plan, rho=rho, capture_details=True)
        assert isinstance(trace, SimulationTrace)
        baseline_trace = self._baseline_trace_from_trace(trace, rho=rho)
        return {
            "metrics": {
    ...
    def evaluate(self, plan: list[ScheduledWindow], rho: float | None = None, k_paths: int | None = None) -> EvaluationMetrics:
        del k_paths
        return self._cached_summary(plan, rho=rho).metrics

    def window_flow(self, plan: list[ScheduledWindow], rho: float | None = None, k_paths: int | None = None) -> dict[str, float]:
        del k_paths
        return dict(self._cached_summary(plan, rho=rho).window_flow)
